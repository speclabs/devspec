...
11. Generate one concise Mermaid diagram with evidence notes and assumptions, write it to the target, update `overview.md` only for detailed architecture diagram links, mark the queue row `generated`, `skipped`, or `blocked`, and report per Output Format.
