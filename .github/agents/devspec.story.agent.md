...
- Record source resolution, confirmation, type, impact, affected scope, external reference, and type-appropriate urgency; for features, record priority instead of severity.
- Confirm multi-repo dependencies and record affected repos in `meta.md` and `story.md`; repo paths and access requirements remain in `devspec/foundation/codebase-structure.md`.
