...
- When finalizing an accepted change request, set `Resume State` `Current item` to the active `CR-###` and append CR-scoped requirement coverage, readiness, implementation brief, validation plan, and blocker rows. Preserve prior baseline and prior CR rows.
- Run the Readiness Gap Scan before setting `finalize.md` to `ready`.
- Create or update `Requirement Coverage` for every completeness dimension in the current baseline or active change-request scope. Resolve facts from durable artifacts and repository evidence before asking the user, and apply the Technology-Neutral Safety Baseline when its boundary is applicable.
- Use `RC-###` for baseline coverage and `CR-###-RC-###` for change-request coverage. Do not backfill or rewrite completed baseline coverage.
- Set `Readiness Assessment` status to `ready` only when every applicable Requirement Coverage row and every required readiness gate is `ready` or `not applicable`; otherwise set it to `not ready`.
- Keep `Readiness Assessment` identifiers separated: record only `RC-###` or `CR-###-RC-###` IDs in `Blocking coverage`, and only `RG-###` IDs in `Blocking gates`.
...
- Keep `finalize.md` implementation-oriented: requirement coverage, readiness gates, implementation brief, validation plan, and blockers.
...
- Evaluate readiness gates as summaries of specific checks for scope, acceptance criteria, dependencies or repository readiness, type-specific requirements, and validation or delivery risk. Record detailed Readiness Gap Scan outcomes in `Requirement Coverage`, then promote task-planning facts to `Implementation Brief` and proof expectations to `Validation Plan`; do not create a separate speculative audit artifact.
- Record coverage dependencies before selecting a question. A question is eligible only when every referenced prerequisite coverage row is `ready` or `not applicable`; among eligible questions choose the highest priority, then highest implementation impact and uncertainty.
- After an answer is applied to its owning artifact, rerun completeness analysis and recompute all dependent coverage rows. Reopen invalidated `ready` rows, mark obsolete rows `not applicable`, and select the next eligible question.
- When any eligible requirement gap remains, set readiness to `not ready`, record its coverage ID in `Blocking coverage`, record the summarized affected gate IDs in `Blocking gates`, record the blocking action in `Readiness Assessment`, update `meta.md` and `finalize.md` resume state, and hand off to `/devspec.clarify` with exactly one active question.
...
5. Run the Readiness Gap Scan and Requirement Completeness Matrix for the current item (`baseline` or active `CR-###`), including foundation and architecture alignment, safety-baseline applicability, dependencies, evidence, routing, and validation coverage.
6. Recompute dependent rows after every recorded answer, select only the highest-priority dependency-eligible blocker, and resolve it through a structured `selection` or `clarification` question following the Interactive Question Pattern; use `/devspec.clarify` when a separate clarification handoff is needed.
...
- Ready status
- Key requirement coverage, dependency, scope, foundation or architecture alignment, readiness, validation, and blocker changes
